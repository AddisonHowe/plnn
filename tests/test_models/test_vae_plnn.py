"""Tests for Variational Autoencoder PLNNs.
...
ENC_LOGVAR_WS = np.array([
    [0, 0, 0],
    [0, 0, 0],
], dtype=float)
ENC_LOGVAR_BIAS = np.array([
    [-np.inf, -np.inf],
], dtype=float)
...
def get_model(
        ws, wts, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, decoder_wts, dtype, 
        sigma=0, 
        seed=None, 
        ncells=4, 
        dt=0.1, 
    ...
    model = model.initialize(
        ...
        init_tilt_bias_args=[],
        init_enc_mlp_weights_method='explicit',
        init_enc_mlp_weights_args=[enc_mlp_wts],
        init_enc_mlp_bias_method='none',
        init_enc_mlp_bias_args=[],
        init_enc_mu_weights_method='explicit',
        init_enc_mu_weights_args=[enc_mu_wts],
        init_enc_mu_bias_method='none',
        init_enc_mu_bias_args=[],
        init_enc_logvar_weights_method='explicit',
        init_enc_logvar_weights_args=[enc_logvar_wts],
        init_enc_logvar_bias_method='explicit',
        init_enc_logvar_bias_args=[ENC_LOGVAR_BIAS],
...
class TestBatchedCoreLandscapeMethods:
    ...
    @pytest.mark.parametrize('ws, wts, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, dec_ws, sigparams, t, x, f_exp, f_shape_exp', [
        [[W1, W2, W3], [WT1], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS,
    ...
    def test_f(self, dtype, rtol, atol, ws, wts, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, dec_ws, sigparams, t, x, 
               f_exp, f_shape_exp):
        sigparams = jnp.array(sigparams, dtype=dtype)
        t = jnp.array(t, dtype=dtype)
        x = jnp.array(x, dtype=dtype)
        model = get_model(ws, wts, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, 
                          dec_ws, dtype, ncells=x.shape[1])
    ...
    @pytest.mark.parametrize('ws, wts, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, dec_ws, t, x, sigma, g_exp, g_shape_exp', [
        [[W1, W2, W3], [WT1], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, 
         [0],  # 1 batch
         [[[0, 1]]], 0.01, 
         [[[0.01, 0.01]]], (1, 1, 2)
    ...
    def test_g(
            self, dtype, rtol, atol, ws, wts, enc_mlp_wts, enc_mu_wts, 
            enc_logvar_wts, dec_ws, t, x, sigma, g_exp, g_shape_exp
    ):
        model = get_model(ws, wts, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, 
                          dec_ws, dtype, sigma)
        t = jnp.array(t, dtype=dtype)
    ...
    @pytest.mark.parametrize("ws, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, dec_ws, x, phi_exp, phi_shape_exp", [
        [[W1, W2, W3], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, 
         [[[0, 0]]], [[0.0]], (1,1)],
        [[W1, W2, W3], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, 
    ...
    def test_phi(self, dtype, rtol, atol, ws, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, dec_ws, x, phi_exp, phi_shape_exp):
        model = get_model(ws, [WT1], enc_mlp_wts, enc_mu_wts, enc_logvar_wts, dec_ws, dtype=dtype)
        x = jnp.array(x, dtype=dtype)
    ...
    @pytest.mark.parametrize("ws, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, dec_ws, x, grad_phi_exp, shape_exp", [
        [[W1, W2, W3], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, [[[0, 0]]], [[[6.0, 14.0]]], (1, 1, 2)],
        [[W1, W2, W3], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, [[[0, 1]]], [[[-3.5586, -0.83997]]], (1, 1, 2)],
        [[W1, W2, W3], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, [[[1, 0]]], [[[1.11945, 2.71635]]], (1, 1, 2)],
        [[W1, W2, W3], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, [[[1, 1]]], [[[-0.00409335, 0.0116181]]], (1, 1, 2)],
        [[W1, W2, W3], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, 
         [[[1, 1]], [[1, 0]]], 
    ...
    def test_grad_phi(self, dtype, rtol, atol, ws, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, dec_ws, x, grad_phi_exp, shape_exp):
        model = get_model(ws, [WT1], enc_mlp_wts, enc_mu_wts, enc_logvar_wts, dec_ws, dtype)
        x = jnp.array(x, dtype=dtype)
    ...
    @pytest.mark.parametrize("ws, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, dec_ws, x, grad_phi_exp", [
        [[W1, W2, W3], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, [[[0, 0]]], [[[6.0, 14.0]]]],
        [[W1, W2, W3], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, [[[0, 1]]], [[[-3.5586, -0.83997]]]],
        [[W1, W2, W3], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, [[[1, 0]]], [[[1.11945, 2.71635]]]],
        [[W1, W2, W3], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, [[[1, 1]]], [[[-0.00409335, 0.0116181]]]],
        [[W1, W2, W3], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, 
        [[[1, 1]], [[1, 0]]], 
    ...
    def test_grad_phi_with_shuffle(
            ...
            ws, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, dec_ws, x, grad_phi_exp, ncells, seed
    ):
    ...
    @pytest.mark.parametrize('wts, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, dec_ws, sigparams, t, grad_tilt_exp, shape_exp', [
        [[WT1], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, [[[5, 0, 1],[5, 1, -1]]], [0], [[4, 1]], (1, 2)],
        [[WT1], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, [[[5, 0, 1],[5, 1, -1]]], [10], [[-2, -2]], (1, 2)],
        [[WT1], ENC_MLP_WS, ENC_MU_WS, ENC_LOGVAR_WS, DEC_WS, 
    ...
    def test_grad_tilt(
            self, dtype, rtol, atol, wts, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, dec_ws, 
            sigparams, t, grad_tilt_exp, shape_exp
    ):
        sigparams = jnp.array(sigparams, dtype=dtype)
        model = get_model([W1,W2,W3], wts, enc_mlp_wts, enc_mu_wts, enc_logvar_wts, dec_ws, dtype, 
...
def test_steady_state_distribution(
    ...
    model = model.initialize(
        ...
        init_phi_weights_args=[[[[1,0],[0,1],[1,1]],
                                ...
                                [[c1,c2,c3]]]],
        init_enc_mlp_weights_method='explicit',
        init_enc_mlp_weights_args=[ENC_MLP_WS],
        init_enc_mu_weights_method='explicit',
        init_enc_mu_weights_args=[ENC_MU_WS],
        init_enc_logvar_weights_method='explicit',
        init_enc_logvar_weights_args=[ENC_LOGVAR_WS],
        init_enc_logvar_bias_method='explicit',
        init_enc_logvar_bias_args=[ENC_LOGVAR_BIAS],
        init_dec_weights_method='explicit',
